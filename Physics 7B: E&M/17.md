...
-   Process Steps:
    ...
    2. Mount reacts with an opposite reactive force, keeping it at it's original legth:
...
-   **[Charle's Law](https://en.wikipedia.org/wiki/Charles%27s_law)**
...
-   Avogadro's hypothesis:
    ...
    -   $N_a$ is avogadro's number: the number of particles that are contained in one mole of gas
    -   $N_a = 6.022 \cdot 10^23 \text{particles/mole}$
    -   Therefore, if $N$ is the number of molecules of a gas sample, then
        $$N = n\cdot N_A \to PV = \frac{N}{N_A}RT = Nk_bT$$
