...
1. **Select a reference (ground) node** .$u_1 = 0$
{{< columns >}}<!-- mathjax fix -->
...
$$I_{I_s} = I_s$$
and we have .$u_1 = V_s$Applying Ohm’s law to the two resistors, we find that
{{< /columns >}}
...
{{< columns >}}<!-- mathjax fix -->
- Sometimes we want to solve for branch currents: These are easily obtained from the node voltages and
element equations. 
<---><!-- mathjax fix -->
