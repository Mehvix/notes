---
title: "Week 7: 2D Touchscreens & Superposition + Equivalence"
weight: 7
---
...
- Equivalence tells us _nothing_ about the power in a circuit and one should be careful not to assume it does.
...
{{< columns >}}<!-- mathjax fix -->
1. **Find** .$V_{Th}$: Connect an **open circuit** across the two output terminals and measure the voltage across them. This measured .$V_{oc}$ equals .$V_{Th}$.
...
{{< columns >}}<!-- mathjax fix -->
> Any linear two-terminal circuit can be replaced by a current source and a single resistor in parallel -- [wiki](https://en.wikipedia.org/wiki/Norton%27s_theorem)
<---><!-- mathjax fix -->
...
- For each independent source .$k$ (either voltage source or current source)
    - Set all other independent sources to .$0$
        - _Voltage source:_ replace with a wire
        - _Current source:_ replace with an open circuit
