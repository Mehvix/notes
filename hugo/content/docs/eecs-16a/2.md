...
| ---------- | ------ | --------------- | -------------------------- |
| Current    | I      | Amperes, .$A$   | Flows through an element   |
| Voltage    | V      | Volts, .$V$     | Applied across an element  |
| Resistance | R      | Ohms, .$\Omega$ | Opposition to current flow |

- These are all related via **Ohm's Law:** .$V_\text{element} = R \cdot I_\text{element}$
