...
{{< columns >}}<!-- mathjax fix -->
- .$V_\text{elem} = 0$ -- A wire is an ideal connection with zero voltage across it.
...
- This element is the dual of the wire.
    ...
    - .$I_\text{elem} = 0$ -- No current is allowed to flow through an open circuit.
<---><!-- mathjax fix -->
...
- The unit of .$R$ is Volts/Amperes, or more commonly “Ohms” .$\Omega$.
