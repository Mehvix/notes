...
- For a 3 × 3 matrix, the shape is a parallelipiped, and we form hyper-volumes in higher dimensions.
...
$$A \vec x = \lambda \vec x \Longrightarrow (A - \lambda I_n) \vec x = \vec 0_n \Longrightarrow A' \vec x = \vec 0_n \label{a}\tag{1}$$
...
- For the 2 x 2 case determinant is 
...
- If the eigenvectors are _distinct_, they form an **eigenspace**
    $$E_\lambda = \text{null}(\lambda I_n - A) = \text{null}(A')$$
    ...
    - Exactly like the concept of span; the eigenspace is a subspace, the span of all eigenvectors for that eigenvalue (note that this includes .$\vec 0$)
...
- We know that a steady state .$\vec x^*$ of a transformation matrix .$A$ is defined to be such 
    ...
    - Therefore, to solve for the steady-state of a system represented by .$P$, we solve eq .$\eqref{a}$, substituting .$\lambda = 1$
...
- Over time, upon repeated applications of .$A$, we accumulate factors of .$\lambda$; ultimately, .$A^n \vec x = \alpha (\lambda^n \vec x)$ we can see derived to the right
<---><!-- mathjax fix -->
$$\vec x (0) = \alpha \vec v$$
...
- Based on this pattern, we notice the following behaviors based on the value .$\lambda$ as .$n \to \infty$:
    - .$\lambda > 1: \vec x (n) \to \infty$ (exponential growth).
    - .$\lambda = 1: \vec x (n) \to k\vec v$ (steady-state).
    - .$0 < \lambda < 1: \vec x (n) \to \vec 0$ (exponential decay).
    - .$\lambda = 0: \vec x (n) = 0 \vec v = \vec 0$ (instantaneous disappearance).
...
\end{bmatrix}^{-1} \vec x (0)$$
    ...
    - Let’s assume that we drop any terms corresponding to .$\alpha_i = 0$ since those terms are, well, zero. Then, with what remains, we can make some intuitive observations:
        - (diverge) Any .$|\lambda_i | > 1: \vec x (n) \to \infty$. Even if other components in the linear combination decay, the state itself "blows up" to .$\infty$ as this component overshadows all others.
        - (diverge) Any .$|\lambda_i| = −1: \vec x (n) \to ?$ because that component oscillates forever.
        - (converge) All .$−1 < \lambda_i \leq 1: \vec x (n) \to \vec x^\*$, steady-state! Each .$i$th term either decays to zero (.$|\lambda_i| \leq 1$) or stays the same (.$|\lambda_i| = 1$), such that .$\vec x^\* = \sum_{i,\lambda_i=1} \alpha_i \vec v_i$. 
