...
![Maxwell Distribution](https://useruploads.socratic.org/0Pwc9eB1Tn2EFOUeE40A_maxwells-distribution-of-molecular-speeds-html-2c71e084.gif)

$$f(v) = 4\pi N \bigg( \frac{m}{2 \pi k_b T}\bigg)^{3/2} v^2 e^{ -\frac{1}{2} \cdot \frac{mv^2}{k_b T}}$$

- Recognize that .$f(v) \propto T^{-3/2}, v^2$ and **exponentially** .$KE/T$
...
$$v_{avg} = 1.60 \sqrt{\frac{k_bT}{m}}$$
$$v_{rms} = \sqrt{3} \sqrt{\frac{k_bT}{m}}$$
...
![Phase Change Graph](https://s3-us-west-2.amazonaws.com/courses-images-archive-read-only/wp-content/uploads/sites/222/2014/12/20104458/Figure_14_05_04.jpg)
...
- Molecules in liquid are heald tightly together with attractive forces
- Some molecules may momentarily leave the liquid if their velocity is fast enough
    - If velocity isn't too large, the particle will be pulled back to the liquid surface
    - If velocity is large enough, the particle will leave the liquid and enter gas form
...
![Phase Change Graph](https://upload.wikimedia.org/wikipedia/commons/thumb/3/34/Phase-diag2.svg/620px-Phase-diag2.svg.png)
...
- _Partial Pressure_ is the pressure a single gas would excert by itself
...
1. Finite size of molecules (before we assumed seperation .$\gg$ diameter, ignoring density)
    - Since gas particles aren't negligle in size, we can't use all of our volume
    - Particles are solid spheres that can't get closer than .$2r$ to one another
        - That means .$V$ is over-estimated, so there are more collisions than expected
        - For that reason, pressure is higher than estimated with the ideal gas law
    - The unavaliable volume due to particles, .$b$, depends on the .$n$umber of moles
      $$ P(V-nb) = nRT \to P\bigg(\frac{V}{n} - b\bigg) = RT$$
2. Forces between molecules (before we assumed that forces only played an effect in collisions)
    - Electric attractive forces aren't 0
        - Particles towards the edge are slowed down by the other particles attractive forces
        - For that reason, our pressure is lower than estimated with the ideal gas law
    - "Slow down" is propotional to the gas density
...
- Note that these equations aren't amazing, but they're the best generalization we can do to show the relation
- With low densities, .$a\big/(V/n)^2 \ll P$ and .$b \ll V/n$ so Van der Waals equation reduces to the ideal gas law

## 18.6 Mean Free Path

- Molocules bumb into each other a lot which slow them down
- _Mean free path_: Average distance between collisions is proportional to .$\rho ^{-1}, r^{-1}$
    $$l_m = \frac{1}{4\pi \sqrt{2}r^2 (N/V)}$$

## 18.7 Diffussion

- Particles diffuse from high to low concentrations until equilibrium is reached (when .$\rho$ is constant througout)
- Given a tube with a cross section area .$A$, two concentrations, .$C_1$ and .$C_2$, seperated by .$\Delta x$, we can write the rate of diffusion, .$J$, as
    $$J = DA \frac{C_1 - C_2}{\Delta x} = DA \frac{dC}{dx} \ \ \text{[Fick's Law]}$$
- .$D$ is the diffusion constant
    - Varies with temperature, viscocity, and particle size
