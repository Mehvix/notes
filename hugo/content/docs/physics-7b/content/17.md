...
$$T(^\circ C) = \frac{5}{9}(T(^\circF)-32)$$

$$T(^\circ F) = \frac{9}{5}(T(^\circC)) + 32$$
...
- Equations (assuming a constant volume .$V$ )
    ...
    - .$\alpha$ has the units of .$(^\circ C)^{-1}$
...
- Weird water property
    - .$0 - 4 ^\circ C$ : Water increases in density .$\rho^+$ .$\to$ decreases in volume .$V^-$
    - .$4^\circC +$ : Water acts "normally": increase in volume .$V$ proportional to temperature .$T$
...
![Ideal Gas Law Relation Equation](https://upload.wikimedia.org/wikipedia/commons/thumb/6/62/Ideal_gas_law_relationships.svg/300px-Ideal_gas_law_relationships.svg.png)

- Assume that gasses aren't too dense (so .$P \sim \text{atmospheric pressure}$ ) and that they aren't close to liquefaction (boiling) point either (for oxygen, this is .$~183^\circC$ .
...
![Phase Diagram](https://upload.wikimedia.org/wikipedia/commons/thumb/3/34/Phase-diag2.svg/525px-Phase-diag2.svg.png)
...
- .$P_3 = 4.88\ \text{torr}; T_3 = 0.01^\circC$ for water
