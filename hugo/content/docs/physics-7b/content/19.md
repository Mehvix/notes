...
- Therefore, when considering the change in a system involving heating a substance to a phase change (e.g. boiling at temperature .$T$), we can write:
    $$\Delta Q_{\text{total}} = mc \Delta T + mL$$
...
$$\Delta Q = nC_V \Delta T\ \ \ \text{[Constant Volume]}$$
$$\Delta Q = nC_P \Delta T\ \ \ \text{[Constant Pressure]}$$
...
- We can also relate internal energy to molar specific heat for monatomic gases at constant volumes:
    $$\Delta E_\text{int} = Q_V \Longrightarrow \frac{3}{2}nRT = nC_V\Delta T \Longrightarrow C_V = \frac{3}{2}R$$
