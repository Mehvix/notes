...
- E.x. if we started with state .$A$ and added heat .$Q$ to the system, the system would reach point .$B$
    ...
    - We know .$E_\text{int} = \frac{3}{2}nR\Delta T$, and since .$\Delta T = 0 \rightarrow E_\text{int} = 0$
    - Thus, .$E_\text{int} = Q - W \rightarrow W = Q$
...
- Since .$Q = 0 \rightarrow \Delta E_\text{int} = -W$
...
- **[Isobaric](https://en.wikipedia.org/wiki/Isobaric_process)**: .$\Delta P = 0 \rightarrow Q = \Delta E_\text{int} + W = \Delta E_\text{int} + P\Delta V$. The heat transferred to the system does work, but also changes the internal energy of the system
- **[Isovolumetric](https://en.wikipedia.org/wiki/Isochoric_process)**: .$\Delta V = 0 \rightarrow W = 0 \rightarrow Q = \Delta E_\text{int}$. The thermodynamic process is the addition or removal of heat.
...
- Since .$\Delta E_\text{int}$ is the same for both processes, we can write .$Q_P - Q_V = P \Delta V$
    - With an ideal gas, we know .$V = nRT/P$ so .$\Delta V = nR\Delta T/P$ which we can combine with the prior equations to get:
        $$nC_P\Delta T - nC_V \Delta T = P\bigg(\frac{nR\Delta T}{P}\bigg) \rightarrow C_P - C_V = R$$
- We can also relate internal energy to molar specific heat for monatomic gases at constant volumes:
    $$\Delta E_\text{int} = Q_V \rightarrow \frac{3}{2}nRT = nC_V\Delta T \rightarrow C_V = \frac{3}{2}R$$
...
- **Degrees of Freedom**: The number of independent ways a molecule and posses energy
    - Degrees of freedom depend on the temperature
    - At low temperatures, the only degree of freedom is from translational KE; Diatomic gas: .$C_V = \frac{3}{2}R$
    - At "regular" temperatures, the molecules posses rotation energy. Diatomic gas: .$C_V = \frac{5}{2}R$
    - At higher temperatures, the molecules gain energy associated with their vibrations:
        - One from KE of the molecules vibrating back and forth
        - The second from PE of the vibrational motion (think of this as a spring's PE)
    - Solids:
...
- **Principle of Equipartition of Energy**: Energy is shared equally among degrees of freedom and each degrees has energy .$\frac{1}{2}k_bT$
    - Thus, for a particle with three degrees of freedom (such as a monatomic gas) .$C_V = \frac{3}{2}R$
    - Diatomic gases have five degrees so they have .$C_V = \frac{5}{2}R = 4.97 \text{ cal/(mol K)}$ and have .$E_\text{int} = N(\frac{5}{2}k_BT) = \frac{5}{2}nRT$ where .$n$ is the number of moles and .$N$ is the number of molecules
...
$$PV^\gamma = \text{[constant] for } \gamma = \frac{C_P}{C_V}$$
