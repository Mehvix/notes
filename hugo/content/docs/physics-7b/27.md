...
- By Newton's third law, we can see that a magnet exerts a force on a current-carrying wire
    ...
    $$\vec F = I (\vec l \times \vec B) = I l b \sin\theta \Longrightarrow d \vec F = I (d \vec l \times \vec B)$$
...
{{< columns >}}
> Calculating the torque on a current loop in a magnetic field .$\vec B$. . (a) Loop face parallel to .$\vec B$ field lines; (b) top view; (c) loop makes an angle to .$\vec B$, reducing the torque since the lever arm is reduced. The vector .$\vec \mu$ is the “magnetic moment”.
