...
$$\overline{K} = \frac{3}{2}k_B T \Longrightarrow \frac{1}{2}m\bar{v}^2 = \frac{3}{2}k_B T \Longrightarrow v_{\text{rms}} = \sqrt{\bar{v}^2} = \sqrt{\frac{3k_B T}{m}} $$
...
- Recognize that .$f(v) \propto T^{-3/2}, v^2$ and **exponentially** .$\propto KE/T$
    ...
    - Spread of important values (.$v_p, v_{\text{avg}}, \text{\text{etc.}}$) are spread out further from one another
...
- Ideal gases would have a straight line along a PV/nRT v. P graph
- Real gases vary don't follow this line and vary more from it proportional to size and weight (resulting in higher .$a$ and .$b$ values respectively -- [see 18.5]( {{< relref "#185-van-der-waals-equation-of-state" >}} )
...
1. Finite size of molecules (before we assumed separation .$\gg$ diameter, ignoring density)
    ...
    - Where .$b$ is the volume consumed by 1 mol of gas with the units V/mols
