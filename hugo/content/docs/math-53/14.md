...
$$D_{\hat u} f(x_0, y_0) = \lim_{h\to 0} \frac{f(x_0 + ha, y_0 + hb) - f(x_0, y_0)}{h} = f_x (x,y) a + f_y (x,y) b$$
...
$$\nabla f(x,y,z) = \big\langle f_x(x,y,z) + f_y (x,y,z) + f_z (x,y,z) \big\rangle = \frac{\delta f}{\delta x}\hat i + \frac{\delta f}{\delta y}\hat j$$
{{< /hint >}}
- We can now use the gradient to re-write our direction derivative equation as 
...
- The maximal value is .$D_\hat{u} f(\vec x) = \vert \nabla f \vert$
...
- Let .$\vec r(t)$ be a space curve on the surface .$F(x,y,z) = k$ 
    - Let .$\vec r(t_0) = \langle x_0, y_0, z_0 \rangle$ for some .$t_0$ (the space curve passes through some point which is on the surface)
